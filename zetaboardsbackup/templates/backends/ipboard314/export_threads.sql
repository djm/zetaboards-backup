...
({{ thread.zeta_id }}, '{{ thread.title }}', '{{ thread.subtitle }}', 'open', 0, {{ thread.user.zeta_id }}, {{ thread.date_posted|date:"U" }}, NULL, NULL, 0, '{{ thread.username }}', '', '0', 0, {{ thread.views }}, {{ thread.forum.zeta_id }}, 1, 0, 0, '0', 0, 1, 0, 0, 0, 3, 1, '{{ thread.title|slugify }}', '', '{{ thread.username|slugify }}', 0){% if not forloop.last %},{% else %};{% endif %}
{% endfor %}
