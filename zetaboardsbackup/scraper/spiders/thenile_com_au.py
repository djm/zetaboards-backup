...
class TheNileComAuSpider(BaseSpider):
    name = 'thenile.com.au'
    ...
    def start_requests(self):
        ...
        for book in self.books:
            request = Request("http://www.thenile.com.au/books/%s/" % book.isbn,
                    meta={'book': book})
    ...
    def parse(self, response):
        l = BookLoader(item=BookItem(book=response.request.meta['book']),
                response=response)
        price = l.get_xpath('//span[@class="price"]//strong/text()')
        if price:
